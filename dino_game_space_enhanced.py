...
# Game variables
score = 0
game_speed = 15  # Increased base game speed
high_score = 0
...
class Obstacle:
    def __init__(self, image, type):
        ...
        # Each obstacle now has its own speed
        self.speed_multiplier = random.uniform(0.9, 1.5)  # Varying speeds
        
    def update(self):
        # Use the obstacle's individual speed
        self.rect.x -= int(game_speed * self.speed_multiplier)
...
class SpaceRock(Obstacle):
    def __init__(self, image=None):
        ...
        if self.type == 0:  # Small asteroid
            self.speed_multiplier = random.uniform(1.3, 1.7)  # Faster
        elif self.type == 1:  # Medium asteroid
            self.speed_multiplier = random.uniform(1.1, 1.4)  # Medium speed
        else:  # Large asteroid
            self.speed_multiplier = random.uniform(0.9, 1.2)  # Slower
...
class AlienShip(Obstacle):
    def __init__(self):
        ...
        # Alien ships have erratic speeds
        self.speed_multiplier = random.uniform(1.2, 1.8)  # Generally faster
        self.direction_change_timer = 0
        self.vertical_speed = random.choice([-1, 0, 1])  # Can move up/down slightly
    ...
    def update(self):
        ...
        # Occasionally change vertical direction for more erratic movement
        self.direction_change_timer += 1
        if self.direction_change_timer >= 30:  # Change direction every ~1 second
            self.direction_change_timer = 0
            self.vertical_speed = random.choice([-1, 0, 1])
        ...
        elif self.rect.y > GROUND_HEIGHT - 50:
...
class SpaceCloud:
    def __init__(self):
        ...
        self.speed = random.randint(2, 5)  # Faster clouds
...
def main():
    ...
    speed_increase_rate = 0.2  # How much to increase speed per 100 points
    max_game_speed = 30  # Cap the maximum speed
    ...
    def reset_game():
        ...
        cloud_timer = 0
        point_milestone = 100
        game_speed = 15  # Reset to initial speed
        score = 0
        obstacles = []
        clouds = []
        dino.rect.y = dino.y
    ...
    while run:
        ...
        if not game_over:
            ...
            if score % 100 == 0 and score > 0:
                # Increase game speed but cap it
            ...
            # Generate obstacles - more frequent as score increases
            obstacle_timer += 1
            obstacle_frequency = max(30, 100 - (score // 500))  # Decrease wait time as score increases
            if obstacle_timer >= random.randint(obstacle_frequency // 2, obstacle_frequency):
        ...
        speed_text = font.render(f"Speed: {game_speed:.1f}", True, WHITE)  # Show current speed
